...
impl Broker {
    ...
    pub fn publish(&self, message: Message) -> Result<u64> {
        let mut storage = self
            .storage
            .lock()
            .map_err(|_| SamsaError::broker("Storage lock poisoned"))?;
        let mut offsets = self
            .topic_offsets
            .lock()
            .map_err(|_| SamsaError::broker("Topic offsets lock poisoned"))?;
    ...
    pub fn latest_offset(&self, topic: &str) -> Result<u64> {
        let offsets = self
            .topic_offsets
            .lock()
            .map_err(|_| SamsaError::broker("Topic offsets lock poisoned"))?;
        Ok(*offsets.get(topic).unwrap_or(&0))
    }
}
...
mod tests {
    ...
    fn test_broker_creation() {
        ...
        assert_eq!(broker.latest_offset("test.topic").unwrap(), 0);
    }
    ...
    fn test_publish_single_message() {
        ...
        assert_eq!(offset, 0);
        assert_eq!(broker.latest_offset("test.topic").unwrap(), 1);
    }
    ...
    fn test_publish_multiple_messages() {
        ...
        assert_eq!(offset2, 1);
        assert_eq!(offset3, 2);
        assert_eq!(broker.latest_offset("test.topic").unwrap(), 3);
    }
    ...
    fn test_publish_to_different_topics() {
        ...
        assert_eq!(broker.latest_offset("topic.a").unwrap(), 2);
        assert_eq!(broker.latest_offset("topic.b").unwrap(), 1);
    }
    ...
    fn test_default_trait() {
        ...
        assert_eq!(broker.latest_offset("any.topic").unwrap(), 0);
    }
}
