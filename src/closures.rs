...
/// Type alias for a message filter with a boolean predicate
type BoolMessageFilter = MessageFilter<Box<dyn Fn(&Message) -> bool>>;

/// Create common message filters
pub fn create_message_filters() -> (BoolMessageFilter, BoolMessageFilter, BoolMessageFilter) {
    let large_filter = MessageFilter::new(
        "Large Messages".to_string(),
        Box::new(|msg: &Message| msg.value.len() > 1000) as Box<dyn Fn(&Message) -> bool>,
    );

    let system_filter = MessageFilter::new(
        "System Topics".to_string(),
        Box::new(|msg: &Message| msg.topic.starts_with("system.")) as Box<dyn Fn(&Message) -> bool>,
    );
