...
impl RichMessage {
    ...
    pub fn route_message(&self) -> ProcessingResult {
        match (&self.priority, &self.content, self.topic.as_str()) {
            ...
            (_, MessageContent::Binary(data), _) if data.len() > 10240 => ProcessingResult::Queued,
    ...
    match (&message.content, &message.metadata) {
            ...
            if metadata.contains_key("user_id") && metadata.contains_key("action") =>
        {
            if let (Ok(user_id), Some(action)) =
                (metadata["user_id"].parse::<u64>(), metadata.get("action"))
            {
...
pub fn classify_message(message: &RichMessage) -> String {
    match message {
        ...
        RichMessage {
            ...
            metadata,
            ..
        } if metadata.contains_key("encrypted") => match json.get("type") {
        ...
        RichMessage {
            ...
            timestamp,
            ..
        } if *timestamp < current_timestamp() - 3600 => "Old low-priority message".to_string(),
...
mod tests {
    ...
    fn test_connection_state_machine() {
        let (state, actions) =
            handle_connection_event(ConnectionState::Disconnected, ConnectionEvent::Connect);

        assert!(matches!(state, ConnectionState::Connecting { attempt: 1 }));
        assert!(!actions.is_empty());

        let (state, _) = handle_connection_event(state, ConnectionEvent::Connect);
