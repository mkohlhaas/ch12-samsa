...
fn test_multiple_producers_single_consumer() {
    ...
    producer1
        .send_text("shared.topic", "From Producer 1")
        .unwrap();
    producer2
        .send_text("shared.topic", "From Producer 2")
        .unwrap();
    producer1
        .send_text("shared.topic", "From Producer 1 again")
        .unwrap();
...
fn test_multiple_consumers_independent_positions() {
    ...
    for i in 0..5 {
        producer
            .send_text("test.topic", &format!("Message {}", i))
            .unwrap();
    }
...
fn test_consumer_seek_and_replay() {
    ...
    for i in 0..10 {
        producer
            .send_text("replay.topic", &format!("Message {}", i))
            .unwrap();
    }
...
fn test_high_volume_throughput() {
    ...
    for i in 0..message_count {
        producer
            .send_text("high.volume", &format!("Message {}", i))
            .unwrap();
    }
